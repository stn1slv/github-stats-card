...
Run `.specify/scripts/bash/check-prerequisites.sh --json --paths-only` for `REPO_ROOT`. The feature comes from the argument, never from the script, so the script's own feature resolution is unused here.
