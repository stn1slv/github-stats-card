...
- **Gotcha:** Do not confuse this with `calculate_rank` in the same module, which still returns `S`/`A+`/`A`/`A-`/... for the user stats card. Only the contributor card's per-repo rank lost its modifiers. Ranks are now always one character, so `src/rendering/contrib.py` hardcodes `font-size: 10px` for the rank badge; a future multi-character rank has to bring the scaling back.
